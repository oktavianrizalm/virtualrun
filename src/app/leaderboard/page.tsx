...
export default async function LeaderboardPage() {
  ...
  const { data: activities, error } = await supabase
    .from('activities')
    .select('user_id, distance, profiles(full_name)')

  if (error) {
    console.error(error)
  }

  const leaderboardMap = new Map<string, { name: string, distance: number }>()
  ...
  if (activities) {
    for (const act of activities) {
      ...
      if (!leaderboardMap.has(uId)) {
        leaderboardMap.set(uId, { name, distance: 0 })
      }
  ...
  return (
    <div className="min-h-screen p-6 md:p-12 max-w-5xl mx-auto space-y-12">
      <div className="text-center space-y-4 mt-8 mb-16">
        ...
        <p className="text-zinc-400 text-lg max-w-2xl mx-auto">See who has run the furthest. Stats update automatically via Strava.</p>
      </div>
